...
You can see the breakdown of metrics for each endpoint. This includes how many requests were made and the different latency statistics. This demonstrates that the login (api/auth) request takes an average 108 ms. That seems understandable since it creates a cryptographically signed JWT. The Pizza creation call (api/order) takes 48 ms on average. Since making a pizza requires a call to the JWT Pizza Headquarters that seems reasonable.
