...
So now you are sold on the idea of testing. Hopefully, you will immediately see the value of this practice and start having a hard time writing code without at the same time writing the tests.

However, there is a point of diminishing returns. Most of the problems described above are a result of pushing testing too far. This is usually seen in the debate about the correct amount of code coverage.

Code coverage is defined as the percentage of code lines that are executed, or covered, when your tests are run. The following is an example of code that has a coverage of 71% of the lines.

> ![code coverage](codeCoverage.png)

Obviously this is a problem if you want to make sure that the `remove` operation is properly tested. However, what is the cost of having 100% coverage?

In complex code there is often significant code for supporting error and edge cases that only exist in very extreme situations. These are often times the most difficult cases to test because they require complex setup and teardown of infrastructure, only happen when external systems catastrophically fail, or required special timing or sequences of events. That means you have to make extensive investments and inject special code into your production system to support the testing of these cases.

You many find that for those extreme cases, it is better to actually manually test them instead of doing back flips to automate the testing. You can also take the strategy of waiting until an actual bug is reported for these edge cases before you make an extreme investment.

You should also realize what 100% coverage actually means. It does not mean 100% verification. There are infinite possibilities for different parameters, paths, timings, and configurations to a complex software package. Hitting every line of code does mean that you have hit every permutation that you code can execute.
...
For common web application development it is common to have around a 1:1 test/production code ratio, with somewhere above 80% coverage.
